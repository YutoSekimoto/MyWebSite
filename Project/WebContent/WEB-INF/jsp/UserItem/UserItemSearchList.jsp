		...
		<div class="form-search">
			...
			<form action="UserItemSearchListServlet" method="post">
				...
				<input type="submit" class="btn btn-primary" value="検索"> <a
					href="UserItemTourokuServlet">ユーザー商品新規登録</a>
			</form>
		...
		<div class="item-list">
			...
			<ul>
				...
				<c:forEach var="userItem" items="${userItemList}">
					...
					<li><a
						href="UserItemDetailServlet?id=<c:out value="${userItem.id}" />"
						class="gazou"><img
							src="img/<c:out value="${userItem.file}" />"></a>

						<div class="title-price">
							タイトル<br> <a
								href="UserItemDetailServlet?id=<c:out value="${userItem.id}" />"
								class="title"><c:out value="${userItem.name}" /></a> <br>
							<br> 価格<br>
						...
						<div class="link-operate">
							<br> <br> <a
								href="UserItemDeleteServlet?id=<c:out value="${userItem.id}" />"
								class="btn btn-danger">削除</a> <a
								href="UserItemDetailServlet?id=<c:out value="${userItem.id}" />"
								class="btn btn-primary">詳細</a> <a
								href="UserItemUpdateServlet?id=<c:out value="${userItem.id}" />"
