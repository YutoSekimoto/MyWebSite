					...
					<form action="" method="post">
						...
						<div class="form-group">
							<label for="exampleInputName">ユーザー名</label> <input type="text"
								...
								name="name" value="<c:out value="${userDetail.name}" />"
								required>
						</div>
						<div class="form-group">
							<label for="exampleInputAddress">住所</label> <input type="text"
								class="form-control" id="exampleInputAddress"
								placeholder="Address" name="address" required value="<c:out value="${userDetail.address}" />">
						</div>
						<div class="form-group">
						...
						<input type="hidden" name="id"
							value="<c:out value="${userDetail.id}" />"> <a
							href="UserListServlet" class="btn btn-primary">キャンセル</a> <input
							type="submit" class="btn btn-primary" value="更新" id="update">
					</form>
    ...
    document.addEventListener("DOMContentLoaded" , function(){
        ...
        document.getElementById("update").addEventListener("click" ,function(e){
            ...
            if(password1.item(0).value != password2.item(0).value){
				...
				var alert = document
						.getElementsByClassName("alert alert-danger");
